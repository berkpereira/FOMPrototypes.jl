...
function run_prototype(problem::ProblemData, args::Dict{String, Any},
    ...
    if args["acceleration"] == :krylov
        ws = KrylovWorkspace{Float64}(problem, args["variant"], A_gram, τ, args["rho"], args["theta"], args["accel-memory"], args["krylov-operator"])
    elseif args["acceleration"] == :anderson
        ws = AndersonWorkspace(problem, args["variant"], A_gram, τ, args["rho"], args["theta"], args["accel-memory"], args["anderson-period"])
    else
        ws = NoneWorkspace(problem, args["variant"], A_gram, τ, args["rho"], args["theta"])
    end
